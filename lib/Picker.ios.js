    ...
    render() {
        ...
        return (
            <PickerIOS
                ...
                selectedValue={options && selectedOption && options.findIndex((item) => transformOption(item) === transformOption(selectedOption))}
                onValueChange={(idx) => {
                  onSelect && onSelect(options[idx], idx)
               }}
            >
                {
                    options && options.map((item,id) => <Item label={transformOption(item)} value={id} key={id} />)
                }
            </PickerIOS>
        );
    }
